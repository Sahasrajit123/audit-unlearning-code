...
class Trainer:
    ...
    def __init__(
        ...
        # Initialize stop conditions (used for contractive_coefficients)
        ...
        if self.config["unlearning"]["algorithm"] == "iteration":
            ...
            # Optional stop conditions: stop after N steps or N epochs regardless of delta
            self.stop_steps = algo_config.get("stop_steps", None)
            self.stop_epoch = algo_config.get("stop_epoch", None)
            assert (
        ...
        elif self.config["unlearning"]["algorithm"] == "ascent_descent":
            # Alternating ascent on forget / descent on retain; see unlearn_ascent_descent.py
            pass
        elif self.config["unlearning"]["algorithm"] == "retain_finetune":
            # Retain-only finetuning unlearning; see unlearn_retain_finetune.py
            pass
        else:
    ...
    def train_setup(self) -> train_state.TrainState:
        ...
        total_train_steps = self.train_epochs * self.train_steps_per_epoch
        train_dataset_size = getattr(
            self.train_dataloader, "dataset", None
        )
        train_n = len(train_dataset_size) if train_dataset_size is not None else "?"
        logger.info(
            "train_dataset_size=%s, train_steps_per_epoch=%s, train_epochs=%s, total_train_steps=%s",
            train_n,
    ...
    def set_lr_scheduler(self, phase="training"):
        if self.config[phase]["lr_schedule"] == "constant":
            ...
            if phase == "training":
                decay_steps = max(self.train_epochs, 1) * self.train_steps_per_epoch
            else:
                decay_steps = self.unlearn_epochs * self.unlearn_steps_per_epoch
        ...
        # AK: also for the resnet training decaying learning rate 3 times over training
        # is something that people use to get stoa accuracy I think, but lets have it later.
        else:
    ...
    def should_save_training_checkpoint(self, epoch: int) -> bool:
        save_every = int(
            self.config["training"].get(
                "checkpoint_save_every_n_epochs", self.config.get("save_every", 1)
            )
        )
        first_n = int(
            self.config["training"].get("checkpoint_save_first_n_epochs", 5)
        )
        last_n = int(self.config["training"].get("checkpoint_save_last_n_epochs", 5))
        ...
        if epoch > max(self.train_epochs - last_n, 0):
            return True

        return epoch % save_every == 0

    def _phase_multiple_for(self, num: int, phases) -> int:
        """Return applicable multiple for a step number from threshold phases."""
        applicable = phases[0][1]
        for threshold, multiple in phases:
            if num >= threshold:
                applicable = multiple
        return applicable

    def should_save_unlearning_epoch_checkpoint(self, epoch: int) -> bool:
        """Match cleanup policy for unlearn_epoch_* checkpoints."""
        unlearn_cfg = self.config.get("unlearning", {})
        first_n = int(unlearn_cfg.get("checkpoint_keep_first_n_epochs", 5))
        last_n = int(unlearn_cfg.get("checkpoint_keep_last_n_epochs", 5))
        keep_every = int(unlearn_cfg.get("checkpoint_keep_every_n_epochs", 10))
        keep_zero = bool(unlearn_cfg.get("keep_epoch_zero_checkpoint", False))
        ...
        if epoch <= first_n:
            return True
        if epoch > max(self.unlearn_epochs - last_n, 0):
            return True
        if keep_every > 0 and epoch % keep_every == 0:
            return True
        return False

    def should_save_unlearning_step_checkpoint(self, step: int) -> bool:
        """Match cleanup policy for unlearn_step_* checkpoints."""
        unlearn_cfg = self.config.get("unlearning", {})
        first_n = int(unlearn_cfg.get("checkpoint_keep_first_n_steps", 20))
        keep_zero = bool(unlearn_cfg.get("keep_step_zero_checkpoint", False))
        phases = unlearn_cfg.get(
            "checkpoint_keep_multiple_phases",
            [[0, 10], [1000, 100], [2000, 0]],
        )
        ...
        multiple = self._phase_multiple_for(step, phases)
        return bool(multiple > 0 and step % multiple == 0)
    ...
    def validate(self, state, data_loader, epoch, prefix):
        ...
        # Use monotonic step: during unlearn, wandb global step is train_epochs + 1, +2, ... per batch,
        # so validation uses train_epochs + (unlearn steps so far) instead of raw training step count.
        if epoch_prefix == "unlearn_epoch":
            log_step = self.train_epochs + int(round(epoch * self.unlearn_steps_per_epoch))
        else:
            log_step = int(epoch) if isinstance(epoch, (int, float)) else epoch
        wandb.log(val_metrics, step=log_step)
    ...
    #     epoch: int,
    #     *,
    #     unlearn: bool = False,           # <─ NEW flag
    # ) -> None:
    #     """
    #     Persist model/optim state.
    #     • Training  →  files   checkpoint_00010, checkpoint_00020, …
    #     • Unlearning→  files   unlearn_00001,   unlearn_00002,    …
    #     Names cannot collide because of the prefix.
    #     """
    #     prefix = "unlearn_" if unlearn else "checkpoint_"
    #     checkpoints.save_checkpoint(
    #         ckpt_dir=os.path.abspath(self.ckpt_path),
    #         target=state,
    #         step=epoch,
    ...
    def save_checkpoint(
        ...
        *,
        unlearn: bool = False,
        phase: str = "epoch",   # "step" or "epoch"
    ) -> None:
        """
        Persist model/optim state.
        • Training      → prefix "checkpoint_"
        • Unlearning   → if phase="step"   → prefix "unlearn_step_"
                        if phase="epoch"  → prefix "unlearn_epoch_"

        Arguments:
        number:    either the epoch index or the step index
        unlearn:   whether this is an unlearning checkpoint
        phase:     "step" or "epoch" (only matters if unlearn=True)
        """
        if unlearn:
            if phase not in ("step", "epoch"):
                raise ValueError(f"Invalid phase={phase!r}, must be 'step' or 'epoch'")
            prefix = f"unlearn_{phase}_"
            ###raise ValueError("Testing reached unlearn")
        else:
            prefix = "checkpoint_"
            ##raise ValueError("Testing reached learn")

        # Optional config-only checkpoint retention: write only checkpoints that
        # the cleanup script would retain.
        if unlearn and self.config.get("unlearning", {}).get("save_only_cleanup_retained", False):
            if phase == "epoch" and not self.should_save_unlearning_epoch_checkpoint(number):
                return
            if phase == "step" and not self.should_save_unlearning_step_checkpoint(number):
                return
    ...
    def fit(self) -> train_state.TrainState:
        ...
        for epoch in range(1, self.train_epochs + 1):
            ...
            if self.should_save_training_checkpoint(epoch):
    ...
    def unlearn_setup(self, state: train_state.TrainState) -> train_state.TrainState:
        ...
        if self.config["unlearning"]["init_model_clip_type"] == "none":
            # No init clip or noise: keep trained params, go directly to finetuning
            final_init_params = state.params
        else:
    ...
    def unlearn_sigma_t(self, step):
        if self.config["unlearning"]["algorithm"] == "contractive_coefficients":
            # Ensure sigma is a JAX array for proper handling in JIT context
            return jnp.asarray(self.sigma, dtype=jnp.float32)
        if self.config["unlearning"]["algorithm"] != "iteration":
        ...
        if lambda_t:
            if self.config["unlearning"]["noise_schedule"] == "constant":
                ...
                else:  # AK: in all of these cases only constant learning rate is supported right?
                    # should we just move this check above?
                    # AJ: I thought the plan was to test out with other schedules too
                    ValueError(
                ...
                # AK: is unlearn epsilon - Renyi one?
                # AJ: Yes, renamed variables to indicate that
                denom = (
                ...
                # AK: I couldn't find where did you take this noise schedule from?
                #     In Theorem 2 there is no noise schedule specified. So I wasn't able to check this.
                # YA: I checked that the formula below is correct. I had taken that particular result out of Theorem 2 to the appendix.
                if self.config["unlearning"]["lr_schedule"] == "constant":
                    eta_t = self.lr_schedule(step)
                else:
                    raise ValueError(
        ...
        else:  # no regulatization (if (not lambda_t):)
            # AK: could here also noise schedule be non constant?
            if self.config["unlearning"]["lr_schedule"] == "constant":
                eta_t = self.lr_schedule(step)
            else:
                raise ValueError(
    ...
    def unlearn_step(
        ...
        if self.config["unlearning"]["algorithm"] == "iteration":
            ...
            new_state = state.apply_gradients(grads=total_update)
            ...
            if self.model_clip:
                ...
                clipped_model = new_state.params
    ...
    def _compute_epsilon_tilde(self, epsilon: float, current_delta: float) -> float:
        """
        Compute epsilon_tilde = log(1 + (exp(epsilon) - 1) / current_delta)
        in a numerically stable way to avoid overflow for large epsilon values.
        
        For large epsilon, exp(epsilon) >> 1, so:
          ≈ log(1 + exp(epsilon) / current_delta)
          = log(1 + exp(epsilon - log(current_delta)))
          = log(1 + exp(x)) where x = epsilon - log(current_delta)
        """
        try:
            log_delta = np.log(max(current_delta, 1e-100))
            x = epsilon - log_delta
            
            # Get safe exponent range
            max_exp = np.log(np.finfo(np.float64).max) - 1
            min_exp = np.log(np.finfo(np.float64).tiny) + 1
            
            if x > max_exp:
                # exp(x) would overflow, so log(1 + exp(x)) ≈ x
                epsilon_tilde = x
            elif x < min_exp:
                # exp(x) would underflow, so log(1 + exp(x)) ≈ 0
                epsilon_tilde = 0.0
            else:
                if x > 0:
                    # Use log1p(exp(-x)) for numerical stability when x > 0
                    if -x > min_exp:
                        epsilon_tilde = x + np.log1p(np.exp(-x))
                    else:
                        epsilon_tilde = x
                else:
                    # Use log1p(exp(x)) when x <= 0
                    epsilon_tilde = np.log1p(np.exp(x))
            return epsilon_tilde
        except (OverflowError, ValueError):
            # Fallback: if all else fails, use approximation
            return epsilon - np.log(max(current_delta, 1e-100))
    ...
    def dp_accountant(self, step, sigma_t, eta_t) -> Optional[bool]:
        if self.config["unlearning"]["algorithm"] == "contractive_coefficients":
            if self.stop_unlearning:
                ...
                # No noise: skip epsilon accounting, never stop (gamma=1 so delta stays 1)
                gamma = 1
            elif self.unlearn_grad_clip and self.noise_addition_before_projection:
                epsilon_tilde = self._compute_epsilon_tilde(
                    self.unlearn_epsilon, self.unlearn_current_delta
                )
                gamma = theta_epsilon(
                    epsilon_tilde,
            ...
            # AK: so here you rely on the fact that dp_accountant is called at every iteration...
            #     What if it is not? can you catch this mistake somehow?
            #     Or maybe better to rewrite code to remove this implicit assumption?
            # AJ: Is it really a big assumption? unlearn_one_epoch calls this function at every iteration
            # AK: I guess in this case it is ok, but normally you do not want to design your code that way
            #       as it leads to bugs that are hard to debug. or at least you implement the check that it
            #       is really called at every iteration.
            wandb.log(
                {
                    "gamma": gamma,
            ...
            msg = f"gamma: {gamma}, state_step: {self.logging_state_step_adjustment + step}, delta: {self.unlearn_current_delta}, epsilon: {self.unlearn_epsilon}"
            
            if hasattr(self, "run_dir") and self.run_dir is not None:
                            local_log = os.path.join(self.run_dir, "trainer_local.log")
            ...
            if self.unlearn_current_delta <= self.unlearn_delta:
                if not self.stop_unlearning:
                    ...
                    msg = f"Stopped unlearning (reached delta threshold), delta={self.unlearn_current_delta:.6e} <= target_delta={self.unlearn_delta:.6e}, step={step}"
                    logger.info(msg)
                    if hasattr(self, "run_dir") and self.run_dir is not None:
                            local_log = os.path.join(self.run_dir, "trainer_local.log")
                            with open(local_log, "a") as fh:
            ...
            if self.stop_unlearning:
                ...
                alpha = np.linspace(
                    1.001, 1000, 10000
                )  # YA: better to make the upper limit larger, e.g., np.linspace(1.001, 100, 1000)
                eps_array = (
                    eps_renyi
                ...
                msg = f"Stopped unlearning, Best alpha: {best_alpha}"
                logger.info(msg)
                if hasattr(self, "run_dir") and self.run_dir is not None:
                    local_log = os.path.join(self.run_dir, "trainer_local.log")
                    with open(local_log, "a") as fh:
                ...
                # 2) (eps, delta): ( … )
                msg = f"(eps, delta): ({eps}, {self.unlearn_delta})"
                logger.info(msg)
                if hasattr(self, "run_dir") and self.run_dir is not None:
                    local_log = os.path.join(self.run_dir, "trainer_local.log")
                    with open(local_log, "a") as fh:
    ...
    def unlearn_one_epoch(
        ...
        unlearn_metrics = []
        max_unlearn_step = self.config.get("unlearning", {}).get("max_unlearn_step", None)

        for i, batch in enumerate(self.retain_dataloader):
            # jax.debug.pri)nt("643: state norm {}", optax.global_norm(state.params))
            
            # Check stop conditions for contractive_coefficients BEFORE the step
            if self.config["unlearning"]["algorithm"] == "contractive_coefficients":
                if not self.stop_unlearning:
                    current_local_step = self.logging_state_step_adjustment + state.step
                    # Check stop_steps condition (will stop after this step completes)
                    if self.stop_steps is not None and current_local_step >= self.stop_steps:
                        self.stop_unlearning = True
                        msg = f"Stopped unlearning (HARD CUTOFF), reached stop_steps={self.stop_steps} at step={current_local_step}"
                        logger.info(msg)
                        if hasattr(self, "run_dir") and self.run_dir is not None:
                            local_log = os.path.join(self.run_dir, "trainer_local.log")
                            with open(local_log, "a") as fh:
                                fh.write(msg + "\n")
                        wandb.log({"stop_unlearn": current_local_step, "stop_reason": "stop_steps"})
                    # Check stop_epoch condition
                    elif self.stop_epoch is not None and epoch >= self.stop_epoch:
                        self.stop_unlearning = True
                        msg = f"Stopped unlearning (HARD CUTOFF), reached stop_epoch={self.stop_epoch} at epoch={epoch}"
                        logger.info(msg)
                        if hasattr(self, "run_dir") and self.run_dir is not None:
                            local_log = os.path.join(self.run_dir, "trainer_local.log")
                            with open(local_log, "a") as fh:
                                fh.write(msg + "\n")
                        wandb.log({"stop_unlearn": epoch, "stop_reason": "stop_epoch"})
            
            # If hard cutoff just triggered, setup post_unlearn phase once before first post_unlearn_step.
            if self.stop_unlearning and not self._post_unlearn_setup_called:
                ...
                self._post_unlearn_setup_called = True
            
            if self.stop_unlearning:
                state, metrics = self.post_unlearn_step(state, batch)
                sigma_t = 0
                eta_t = self.lr_schedule(state.step)
                local_step = self.logging_state_step_adjustment+ state.step
                if local_step >=100: ###just using it to save most steps
                    if (local_step % 1000) == 0:
                        self.save_checkpoint(state, local_step, unlearn=True, phase="step")
                else:
                    if (local_step % 50) == 0:
                        try:
                            self.save_checkpoint(state, local_step, unlearn=True, phase="step")
                            ...
                            raise ValueError(f"Failed to save checkpoint step {local_step}")
                            # continue  # skip saving if error occurs
            else:
                ...
                checkpoint_save_more = self.config["unlearning"].get("checkpoint_save_more", False)
                checkpoint_save_more_upto = self.config["unlearning"].get("checkpoint_save_more_upto", 500)
            
                if (local_step % 1 == 0 and local_step <= 30) or local_step%100 == 0 or (checkpoint_save_more and (local_step%10 == 0 and local_step <= checkpoint_save_more_upto)):
                    self.save_checkpoint(
                        state, local_step, unlearn=True, phase="step"
                    )
            ...
            msg = f"epoch: {epoch}, step: {self.logging_state_step_adjustment+ state.step}, metrics: {metrics}, sigma_t: {sigma_t}, eta_t: {eta_t}, unlearn_stop: {self.stop_unlearning}, logging_state_step_adjustment: {self.logging_state_step_adjustment}"
            if hasattr(self, "run_dir") and self.run_dir is not None:
                            local_log = os.path.join(self.run_dir, "trainer_local.log")
                            with open(local_log, "a") as fh:
                                fh.write(msg + "\n")

            if max_unlearn_step is not None and local_step >= int(max_unlearn_step):
                self._max_unlearn_step_reached = True
                logger.info(
                    f"Stopping unlearning early at local_step={local_step} due to max_unlearn_step={max_unlearn_step}"
                )
                break

            # Trigger post_unlearn_setup only if delta was reached (not hard cutoff, which is handled above)
            if unlearn_stop:
                logger.info("Validating after all noise addition")
                self.validate(
                    state, self.val_dataloader, epoch, "Val (Post Noise Addition)"
    ...
    def unlearn(self, state: train_state.TrainState):
        start_time = time.time()
        self._post_unlearn_setup_called = False
        self._max_unlearn_step_reached = False
        l2_norm_params = jnp.sqrt(
            sum(jnp.sum(x**2) for x in jax.tree_util.tree_leaves(state.params))
        )
        ...
        self.save_checkpoint(state, 0, unlearn=True)
        ...
        for epoch in range(1, self.unlearn_epochs + 1):
            ...
            if (epoch + 1) % self.config["save_every"] == 0:
                # 👉 save with distinct prefix so training files stay untouched
                self.save_checkpoint(state, epoch, unlearn=True)
            if self._max_unlearn_step_reached:
                break
        total_time = time.time() - start_time
    ...
    # def prune(self, state: train_state.TrainState) -> train_state.TrainState:
