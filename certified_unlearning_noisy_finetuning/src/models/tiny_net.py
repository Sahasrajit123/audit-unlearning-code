...
class CIFAR100TinyNet(nn.Module):
    num_classes: int
    ...
    def __call__(self, x, train: bool = True):
        ...
        x = nn.Conv(features=64, kernel_size=(3, 3), padding="same", kernel_init=he_init)(x)
        ...
        x = nn.Conv(features=128, kernel_size=(3, 3), padding="same", kernel_init=he_init)(x)
        x = nn.GroupNorm(num_groups=8)(x)
        x = nn.relu(x)
